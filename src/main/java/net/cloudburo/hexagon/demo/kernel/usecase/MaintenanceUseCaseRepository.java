...
public class MaintenanceUseCaseRepository implements MaintenancePort {
    ...
    public User createUser(User user) throws Exception {
        return getPersistencePort().createUser(user);

    }

    public SchemaRegistry getSchemaRegistry() throws Exception{
        if (schemaRegistry == null) {
            registry = SchemaRegistryFactory.getSchemaRegistry(schemaRegistryClassName);
            registry.registerSchema(User.getClassSchema());
        }
        return registry;
    }
}
